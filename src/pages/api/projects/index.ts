...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createProject() {
    ...
    if (body?.compliance_document?.length > 0) {
      const create_compliance_document = body.compliance_document;
      body.compliance_document = {
        create: create_compliance_document,
      };
    } else {
      delete body.compliance_document;
    }
    if (body?.cost_estimation?.length > 0) {
      const create_cost_estimation = body.cost_estimation;
      body.cost_estimation = {
        create: create_cost_estimation,
      };
    } else {
      delete body.cost_estimation;
    }
